...
export default function PengaturanScreen() {
  ...
  useFocusEffect(useCallback(()=>{ setSettings(getPengaturan()); },[]) );
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.header}>
        ...
        <TouchableOpacity style={[s.saveBtn,saved&&{backgroundColor:Colors.success}]} onPress={save}>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.content}>
        ...
        <View style={s.card}>
          {[['nama_toko','Nama Toko'],['alamat','Alamat'],['no_hp','No. HP / WA'],['footer_struk','Footer Struk']].map(([k,label],i,arr)=>(
            <React.Fragment key={k}>
              <View style={s.field}>
                <Text style={s.fieldLabel}>{label}</Text>
                <TextInput style={s.fieldInput} value={settings[k]||''} onChangeText={v=>update(k,v)} placeholderTextColor={Colors.textMuted}/>
              </View>
        ...
        <View style={s.card}>
          {[['Aplikasi','Kasir WarungKu'],['Versi','1.0.0'],['Database','SQLite Offline'],['Mode','🟢 Offline']].map(([l,v],i,arr)=>(
            <React.Fragment key={l}>
              <View style={s.aboutRow}><Text style={s.aboutL}>{l}</Text><Text style={s.aboutV}>{v}</Text></View>
              {i<arr.length-1&&<View style={s.div}/>}
            </React.Fragment>
          ))}
        </View>
...
const s = StyleSheet.create({
  ...
  card:{backgroundColor:Colors.card,borderRadius:14,borderWidth:1,borderColor:Colors.border,marginBottom:16,overflow:'hidden'},
  ...
  aboutRow:{paddingHorizontal:16,paddingVertical:12,flexDirection:'row',justifyContent:'space-between'},
